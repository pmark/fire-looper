...
# Licence CC = BY NC SA
#
# Last update: Jul 10, 2013 release 58
...
#
RELEASE       := 59
RELEASE_PATH  := http://embedxcode.free.fr/release.php?tag=$(BOARD_TAG)&goal=$(MAKECMDGOALS)&ver=$(RELEASE)
...
prepare:
		...
		@if [ -f Utilities/embedXcode_prepare ]; then if [ -n '$(MESSAGE_LINE)' ]; then osascript -e 'tell application "System Events" to if button returned of (display dialog "$(MESSAGE_LINE)" buttons {"Contribute", "Ignore"} with icon POSIX file ("$(UTILITIES_PATH_AS)/TemplateIcon.icns") default button 1 with title "embedXcode" giving up after 5) = "Contribute" then tell application "Safari"' -e 'open location "http://www.embedXcode.weebly.com/contact"' -e 'activate' -e 'end tell'; fi; fi;
